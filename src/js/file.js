...
export class eFile {
    ...
    static downloadData(filename, data, type = "text/plain") {
        try {
            if (!eType.str(filename) || !eType.str(data) || !eType.str(type))
                throw `invalid filename=${eStr.from(filename)}|data=${eStr.from(data)}|type=${eStr.from(type)}`;
    ...
    static downloadBlob(filename, blob) {
        try {
            if (!eType.str(filename) || !eType.blob(blob))
                throw `invalid filename=${eStr.from(filename)}|blob=${eStr.from(blob)}`;
    ...
    static async getImageFromFile(file) {
        try {
            if (!eType.file(file)) throw `invalid file=${eStr.from(file)}`;
            return await eFile.getImageFromUrl(
