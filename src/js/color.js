...
export class eColor {
    ...
    setBrightness(percent) {
        try {
            ...
            return new eColor(
                eNum.clamp(this.#r + add, 0, 255),
                eNum.clamp(this.#g + add, 0, 255),
                eNum.clamp(this.#b + add, 0, 255),
                this.#o
            );
        } catch (err) {
            console.trace(this?.constructor?.name, err);
    ...
    static scaleColors(perc, stops, colors, o) {
        try {
            ...
            if (perc <= Math.min(...stops)) return eColor.parse(colors[0]).setOpacity(o);
            else return eColor.parse(colors[colors.length - 1]).setOpacity(o);
        } catch (err) {
            console.trace(this?.constructor?.name, err);
            return null;
        }
