import eCookie from "./eCookie";
import eDom from "./eDom";
import eList from "./eList";

export default class eTranslation {
    #locales = {};
    ...
    #fillerTag = null;
    //========< contructor
    constructor(locales = [], { fillerTag = null, autoDetect = true, defaultLocale = "en" } = {}) {
        try {
    ...
    #detectLang() {
        ...
        if (!this.locale || !localesCodes.includes(this.locale.code)) {
            ...
            if (localesCodes.includes(browser)) this.#locale = this.locales?.[browser];
            else this.#locale = this.locales?.[localesCodes[0]];
        }
    }
    //========< set
    setLocale(code, { reloadPage = false } = {}) {
        try {
    ...
    //========< translate
    translate(key, { fillers = null, altLocale = null } = {}) {
        try {
            var cc = (altLocale ?? this.locale?.code);
            return (this.locales?.[cc]?.translate(key, { fillers: fillers, fillerTag: this.#fillerTag }) ?? "N/A");
        } catch (err) {
            console.trace(this?.constructor?.name, err);
            return undefined;
        }
...
export class eLocale {
    ...
    // translate
    translate(key, { fillers = null, fillerTag = null } = {}) {
        try {
