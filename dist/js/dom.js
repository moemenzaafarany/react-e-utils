...
export class eDom {
  ...
  static dispatchEvent(element, event = "", {
    ...
    try {
      // isn't win, doc, el
      if (!eType.element(element) && window !== element && document !== element) throw `invalid element=${eStr.from(element)}`;
      // do
  ...
  static listenEvent(element, event = "", callback = (evt, eventName) => {}, {
    ...
    try {
      // isn't win, doc, el
      if (!eType.element(element) && window !== element && document !== element || !eType.func(callback)) throw `invalid element=${eStr.from(element)}|callback=${eStr.from(callback)}`;

      // do
      let controller = new AbortController();
      element.addEventListener(event, evt => {
        if (preventDefault) eDom.eventPreventDefault(evt => callback(evt, event))(evt);else callback(evt, event);
      }, {
        capture: capture,
        once: once,
        passive: passive,
  ...
  static listenEvents(element, events = {}, {
    preventDefault = false,
    capture = false,
    once = false,
    passive = false
    ...
    try {
      ...
      for (let eventName in events) {
        if (eType.func(events[eventName])) controllers[eventName] = eDom.listenEvent(element, eventName, events[eventName], preventDefault, capture, once, passive);
      }
      return controllers;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
  ...
  //========< manipulate dom
  static appendSampleToElement(element, sample, {
    outputEl = el => {},
    sampleEl = el => {},
    childEls = {}
  }) {
  ...
  static populateElement(element, {
    parentEl = el => {},
    childEls = {}
  }) {
    try {
      ...
      for (let k in childEls) element.querySelectorAll(k).forEach(el => {
        try {
          childEls[k](el);
        } catch (err) {
          console.trace(this?.constructor?.name, err);
