...
export class eList {
  ...
  static removeIndexFromArr(array, index) {
    try {
      if (!eType.arr(array) || !eType.num(parseInt(index))) throw `invalid array=${eStr.from(array)}|index=${eStr.from(index)}`;
  ...
  static arrToStringTable(array, columns = {
    id: "id",
    name: o => o["name2"]
  }, rowSep = "\r\n", colSep = "\t") {
    try {
      if (!eType.arr(array) && !eType.obj(columns)) throw `invalid array=${eStr.from(array)}|columns=${eStr.from(columns)}`;
      // strings
      let head = "";
      let body = "";
      // append head
      ...
      for (let i in array) {
        ...
        for (let k in columns) {
          if (row) row += colSep;
          if (eType.func(columns[k])) row += columns[k](obj) ?? " ";else row += obj[columns[k]] ?? " ";
        }
        body += row + rowSep;
      }
      return head + body;
  ...
  static arrClean(array, removeCondition = (index, value) => eType.empty(value)) {
    try {
      if (!eType.arr(array) && eType.func(removeCondition)) throw `invalid array=${eStr.from(array)}|removeCondition=${eStr.from(removeCondition)}`;
      for (var k in array) {
  ...
  static removeKeyFromObj(object, key) {
    try {
      if (!eType.obj(object) || !eType.str(key)) throw `invalid object=${eStr.from(object)}|key=${eStr.from(key)}`;
      delete object[key];
      return object;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
    ...
    try {
      ...
      for (var k in object) {
        ...
        if (eType.obj(object[k])) {
          obj = {
            ...obj,
            ...eList.objFlatten(object[k], sep, k)
          };
        } else {
          let n = k;
