...
export class eStr {
  static pad(value, length, padChar = "0", left = true) {
    try {
      if (eType.null(value) || !eType.num(length) || !eType.str(padChar) || !eType.bool(left)) throw `invalid value=${eStr.from(value)}|length=${eStr.from(length)}|padChar=${eStr.from(padChar)}|left=${eStr.from(left)}`;
