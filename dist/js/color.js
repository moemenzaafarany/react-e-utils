...
export class eColor {
  ...
  constructor(r, g, b, o) {
    try {
      ...
      this.#b = b ?? 0;
      this.#o = o ?? 1;
      this.#brightness = (0.299 * this.#r + 0.587 * this.#g + 0.114 * this.#b) / 255;
    } catch (err) {
  ...
  static parse(color) {
    try {
      ...
      var obj = [0, 0, 0, 1];
      // # F f f f
      if (eColor.colors?.[color]) return eColor.colors[color];else if (color?.match(/^#([a-f0-9]{3})$/i)) {
        // #f0f
      ...
      } else if (color?.match(/^rgba\(([0-9 .]+),([0-9 .]+),([0-9 .]+),([0-9 .]+)\)$/i)) {
        // rgba(255,0,255,100)
  ...
  lighten(perc) {
    try {
      ...
      return new eColor(eNum.clamp(this.#r + (255 - this.#r) * perc, 0, 255), eNum.clamp(this.#g + (255 - this.#g) * perc, 0, 255), eNum.clamp(this.#b + (255 - this.#b) * perc, 0, 255), this.#o);
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  darken(perc) {
    try {
      if (eType.empty(perc)) return this;
      return new eColor(eNum.clamp(this.#r - this.#r * perc, 0, 255), eNum.clamp(this.#g - this.#g * perc, 0, 255), eNum.clamp(this.#b - this.#b * perc, 0, 255), this.#o);
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  fadeOut(perc) {
    try {
      return new eColor(this.#r, this.#g, this.#b, eNum.clamp(this.#o - perc, 0, 1));
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  setBrightness(percent) {
    try {
      ...
      return new eColor(eNum.clamp(this.#r + add, 0, 255), eNum.clamp(this.#g + add, 0, 255), eNum.clamp(this.#b + add, 0, 255), this.#o);
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  setOpacity(o = 1) {
    try {
      return new eColor(this.#r, this.#g, this.#b, eNum.clamp(o, 0, 1));
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  addOpacity(o = 1) {
    try {
      return new eColor(this.#r, this.#g, this.#b, eNum.clamp(this.#o + o, 0, 1));
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  isNeighborColor(color, tolerance = 0.12) {
    try {
      ...
      return Math.abs(this.#r - color.rgba[0]) <= tolerance && Math.abs(this.#g - color.rgba[1]) <= tolerance && Math.abs(this.#b - color.rgba[2]) <= tolerance;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  //========< get contrast
  getContrastColor() {
    return new eColor(255 - this.#r, 255 - this.#g, 255 - this.#b, this.#o);
  }
  //========< get text
  getTextColor() {
    let c = this.#brightness < 0.5 ? 255 : 0;
  ...
  hueToColor(perc, color) {
    try {
      ...
      let rgb = eColor.HSLtoRGB(eNum.stepTo(mHSL.h, dH, cHSL.h), eNum.stepTo(mHSL.s, dS, cHSL.s), eNum.stepTo(mHSL.l, dL, cHSL.l));
  ...
  get getHEX() {
    try {
      return `#${eStr.pad(this.#r.toString(16), 2)}${eStr.pad(this.#g.toString(16), 2)}${eStr.pad(this.#b.toString(16), 2)}`;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  get getHEXA() {
    try {
      return `#${eStr.pad(this.#o.toString(16), 2)}${eStr.pad(this.#r.toString(16), 2)}${eStr.pad(this.#g.toString(16), 2)}${eStr.pad(this.#b.toString(16), 2)}`;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
  ...
  static HSLtoRGB(hue, sat, light) {
    try {
      ...
      return {
        r: eNum.round(eNum.clamp(r, 0, 255), 2),
        g: eNum.round(eNum.clamp(g, 0, 255), 2),
        b: eNum.round(eNum.clamp(b, 0, 255), 2)
      };
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
  ...
  //========< generate colors
  static generateColorList(count = 1, {
    saturation = 100,
    lightness = 50
  } = {}) {
    try {
      var colors = [];
      for (let i = 0; i < count; i++) colors.push(eColor.indexColor(i, saturation, lightness, count));
      return colors;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return null;
  ...
  static indexColor(index, {
    ...
    try {
      ...
      let rgb = eColor.HSLtoRGB(index * (360 / colors) % 360, saturation, lightness);
  ...
  static scaleColors(perc, stops, colors, o) {
    try {
      ...
      if (perc <= Math.min(...stops)) return eColor.parse(colors[0]).setOpacity(o);else return eColor.parse(colors[colors.length - 1]).setOpacity(o);
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return null;
    }
