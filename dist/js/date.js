export class eDate {
  ...
  constructor( /**/
    ...
    if (args.length === 0) this.date = new Date(Date.now());else if (args.length === 1) {
      if (eType.eDate(args[0])) this.date = new Date(args[0].date);else this.date = new Date(args[0]);
    } else if (args.length > 1) {
      this.date = new Date(args[0] ?? 0, (args[1] ?? 1) - 1, args[2] ?? 1, args[3] ?? 0, args[4] ?? 0, args[5] ?? 0, args[6] ?? 0, args[7] ?? 0, args[8] ?? 0);
    }
    if (!this.date || this.date === "Invalid Date" || isNaN(this.date)) throw "Invalid Date";
  }
  static parseDate(value, format = 'ymd', includeLastYear = true) {
    // ymd|dmy|mdy|serial
    try {
      switch (format) {
        case "ymd":
  ...
  static monthNoText(month = 0, short = true) {
    const nameShort = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sept", "Oct", "Nov", "Dec"];
    const nameLong = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    try {
      // short
  ...
  getFormated(format = "Y-m-d H-i-s") {
    ...
    try {
      ...
      for (var k2 in formats) {
        if (str.includes(`{${k2}}`)) str = str.replaceAll(`{${k2}}`, formats[k2](this));
      }
      return str;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
  ...
  static periodsIntersect(startDate1, endDate1, startDate2, endDate2, time = "Ymd", firstWeekday = null) {
    try {
