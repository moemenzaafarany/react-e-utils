export class eImageEditor {
  ...
  async #getSourceBlob() {
    return new Promise((resolve, reject) => {
      ...
      this.#context.drawImage(this.#source.image, 0, 0, this.#canvas.width, this.#canvas.height);
  ...
  async setWidth(width, keepRatio = false) {
    if (eType.num(width)) this.#edits.width = width;else this.#edits.width = this.#source.width;
    if (keepRatio) this.#edits.height = this.#edits.width / this.#source.ratio;
    this.#apply();
  }
  async setHeight(height, keepRatio = false) {
    if (eType.num(height)) this.#edits.height = height;else this.#edits.height = this.#source.height;
  ...
  async #apply() {
    return new Promise(resolve => {
      ...
      // draw image
      this.#context.drawImage(this.#source.image, -(imageW / 2), -(imageH / 2), imageW, imageH);
      this.#canvas.toBlob(async b1 => {
        ...
        this.#canvas.toBlob(async b2 => {
          ...
          this.#canvas.toBlob(async blob => {
            ...
            resolve();
          }, this.#edits.quality ? "image/jpeg" : "image/png", this.#edits.quality);
        }, this.#edits.quality ? "image/jpeg" : "image/png", this.#edits.quality);
      });
    });
  }
  ...
  getAsFile() {
    ...
    return new File([this.#output.blob], arr[arr.length - 1], {
  ...
  static async getImageFromFile(file) {
    try {
      ...
      return await eImageEditor.getImageFromUrl(await eFile.getContentAsDataURL(file));
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
    }
