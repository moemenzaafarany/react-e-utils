...
export class eUrl {
  ...
  static setUrl(href = null, {
    ...
    try {
      const url = new URL(href ?? window.location.href);
      if (eType.obj(params)) url.href = eUrl.setParams(params, url.href);else url.href = eUrl.clearParams(url.href);
      if (eType.str(hashPath)) url.href = eUrl.setHash(hashPath, hashValues, url.href);else url.href = eUrl.clearHash(url.href);
      return url.href;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return undefined;
  ...
  static getHashValues(pattern, href) {
  ...
  static setHash(path, values = {}, href) {
    // "path like section1 section/:id"
    try {
...
export class eCookie {
  ...
  static del(name) {
    try {
      if (!eType.str(name)) return false;
      document.cookie = `${name}=${null};expires=${new Date(Date.now() / 2).toUTCString()};path=/`;
      return true;
    } catch (err) {
      console.trace(this?.constructor?.name, err);
      return false;
